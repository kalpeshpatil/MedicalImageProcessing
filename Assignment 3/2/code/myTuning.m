...
% [noise_real, noise_imag] = find_noise(kSpaceData,x1,y1,x2,y2);
...
for u=1:length(alpha_array)
    for v=1:length(gamma_array)
        ...
        fig=figure;set(gcf, 'Position', get(0,'Screensize')); 
        imshow(mat2gray(abs(ifft2(kSpaceData)))); title(['Noisy Image for ' ,prior_type,' prior with alpha = ',num2str(alpha),'and gamma=' ,num2str(gamma)]);
        saveas(fig,['../images/Huber_Tuning/Optimal_Denoised_Image for Brain ',prior_type,' prior with alpha ' num2str(alpha),'with gamma ',num2str(gamma),'.jpg'],'jpg');
        close(fig);
...
for u=1:length(alpha_array)
    for v=1:length(gamma_array)
        ...
        i =1;
        objective_array=[];
        Current_Iterative_Image = ift2(kSpaceData;
        alpha = alpha_array(u);
        gamma = gamma_array(v);
        [alpha,gamma]
        ...
        fig=figure;set(gcf, 'Position', get(0,'Screensize')); 
        imshow(mat2gray(abs(kSpaceData))); title(['Noisy Image for ' ,prior_type,' prior with alpha = ',num2str(alpha),'and gamma=' ,num2str(gamma)]);
        saveas(fig,['../images/Disc_adapt_function_Tuning/Optimal_Denoised_Image for Brain ',prior_type,' prior with alpha ' num2str(alpha),'with gamma ',num2str(gamma),'.jpg'],'jpg');
        close(fig);
        ...
        title('Denoised Image')
        subplot(1,2,1)
        imshow(mat2gray(abs(kSpaceData)));
        title('Noisy Image')
...
for u=1:length(alpha_array)
    s = 0.1; 
    i =1;
    Current_Iterative_Image = kSpaceData;
